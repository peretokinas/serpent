...
<div class="lk-content lk-content__flex lk-favorites">
  <?php
    //Избранные товары - Определяем кол-во 
    $count_fav=0;
    if (isset($arResult["FAVORITES"])) {
      if (is_array($arResult["FAVORITES"])) {
        $count_fav=count($arResult["FAVORITES"]);
      }
    }
  ?>
  <?php if($count_fav!=0):?>
    ...
    ?>
  <?php else:?>
    <div class="no_found_prod_on_lk">Избранных товаров - не найдено</div>
  <?php endif;?>
</div>